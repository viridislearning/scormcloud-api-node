...
RegistrationService.prototype.processRegistrationListResults = function(courseId, r, callback){
	var data = r.registrationlist
...
RegistrationService.prototype.getCohortListResults = function(callback){
	var method = new RequestMethod(this.configuration_, {
	    method: RegistrationService.serviceMethods.getRegistrationListResults,	    
	    resultsformat: 'course',
	    dataformat: 'xml'
		...
		if(r.isOk()){
			...
			for(week in cohorts){
				...
				_.each(cohorts[week].registrations, function(reg){
					...
					// a difference between 15 and 36 days adds to week3_8 bucket
					if(diff > 14 && diff <= 36){
						buckets['week3_8']++;
					}
					// a difference greater than 36 days adds to week9plus bucket
					if(diff > 36){
						buckets['week9plus']++;
					}
				});
...
RegistrationService.prototype.processCohortListResults = function(r, callback){
	var data = r.registrationlist
	var cohorts = {};
	// Create the cohorts
	_.each(data[0].registration, function(reg){
	...
	for(week in cohorts){
		...
		_.each(cohorts[week].registrations, function(reg){
			...
			// a difference between 15 and 36 days adds to week3-8 bucket
			if(diff > 14 && diff <= 36){
				buckets['week3_8']++;
			}
